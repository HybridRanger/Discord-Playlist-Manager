...
async function createServerAbrStream(track, innertube, ext) {
  ...
  const audioStream = new import_node_stream.PassThrough();
...
async function generateToken(innertube) {
  ...
  const poTokenResult = {
    ...await import_bgutils_js.BG.PoToken.generate({
...
var YoutubeiExtractor = class _YoutubeiExtractor extends import_discord_player.BaseExtractor {
  ...
  async #createWithToken() {
    ...
    this.innerTube = await import_youtubei6.default.create({
      ...
      cache: new import_youtubei6.UniversalCache(false),
  ...
  async activate() {
    ...
    this.innerTube = await import_youtubei6.default.create({
      ...INNERTUBE_OPTIONS,
      fetch: (input, init) => defaultFetch(this.context.player, input, init, this.options.proxy),
      cache: this.options.generateWithPoToken ? new import_youtubei6.UniversalCache(false) : void 0,
