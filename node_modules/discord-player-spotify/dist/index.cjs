...
var import_otpauth = require("otpauth");
...
var SpotifyAPI = class {
  ...
  get authorizationKey() {
    return import_node_buffer.Buffer.from(`${this.clientId}:${this.clientSecret}`).toString("base64");
  }
  async requestToken() {
    try {
  ...
  calculateToken(hex) {
    ...
    const bufferToken = import_node_buffer.Buffer.from(token.join(""), "utf8").toString("hex");
    return import_otpauth.Secret.fromHex(bufferToken);
  }
  async getAccessTokenUrl() {
    ...
    const root = (0, import_node_html_parser.parse)(spotifyHtml);
    ...
    const totp = new import_otpauth.TOTP({
      secret: token,
      period: 30,
      digits: 6,
      algorithm: "SHA1"
...
// src/SpotifyExtractor.ts
var SpotifyExtractor = class extends import_discord_player.BaseExtractor {
  constructor() {
    super(...arguments);
    this._credentials = {
  ...
  buildTrack(trackInfo, requestedBy, playlist) {
    return new import_discord_player.Track(this.context.player, {
      ...
      duration: import_discord_player.Util.buildTimeCode(import_discord_player.Util.parseMS(trackInfo.duration_ms || trackInfo.duration || 0)),
      views: 0,
      requestedBy,
      source: "spotify",
      metadata: {
  ...
  buildPlaylist(data, context, type = "playlist") {
    ...
    playlist.tracks = (data.tracks || []).map((trackData) => {
      const track = new import_discord_player.Track(this.context.player, {
        ...
        duration: import_discord_player.Util.buildTimeCode(import_discord_player.Util.parseMS(trackData.duration_ms)),
        views: 0,
